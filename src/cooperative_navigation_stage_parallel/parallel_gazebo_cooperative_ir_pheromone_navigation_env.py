...
class GazeboEnvironment:
    ...
    def step(self, action): # action = [v, w]
        ...
        for i in range(self.robot_num):
            ...
            self.state[i] = list(next_state_pheromone_value) + list(next_state_laser_value) + [next_state_distance_to_goal, next_state_angle_to_goal, next_state_robot_linear_velocity_x, next_state_robot_angular_velocity_z]
    ...
    def reset(self, seed=None):
        ...
        for i in range(self.robot_num):
            ...
            self.state[i] = list(self.pheromone_value[i]) + list(self.laser_value[i]) + [self.distance_to_goal[i], self.angle_to_goal[i], self.robot_linear_velocity[i].x, self.robot_angular_velocity[i].z]
        return self.state
