...
class TrajectoryBuffer:
    ...
    def get_per_trajectory(self, trajectory):
        state = torch.tensor(
            np.stack([x[0] for x in trajectory]), device=self.device, dtype=torch.float32
        )
        action = torch.tensor(
            np.array([x[1] for x in trajectory]), device=self.device, dtype=torch.float32
        )
        log_prob_old = torch.tensor(
            np.array([x[2] for x in trajectory]), device=self.device, dtype=torch.float32
        )
        reward = torch.tensor(
            np.array([x[3] for x in trajectory]), device=self.device, dtype=torch.float32
        )
        next_state = torch.tensor(
            np.stack([x[4] for x in trajectory]), device=self.device, dtype=torch.float32
        )
        done = torch.tensor(
            np.array([x[5] for x in trajectory]), device=self.device, dtype=torch.int32
        )
