...
class WaypointNavigation:
    ...
    def __init__(self):
        ...
        self.arrival_times = np.empty(0)      # 到着した時間配列
        ...
        marker.pose.position.x = self.goal_pos_x
        marker.pose.position.y = self.goal_pos_y
        ...
        marker.color.b = 0.0
        marker.color.a = 1.0
        print("publish maerker")
        self.marker_pub.publish(marker)



    def InformationMaker(self):
            marker = Marker()
            marker.header.frame_id = "world"
            marker.header.stamp = rospy.Time.now()
            marker.ns = "goal"
