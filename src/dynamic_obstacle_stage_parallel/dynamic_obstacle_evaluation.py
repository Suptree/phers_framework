...
def main():
   ...
   #　例外処理
    if len(sys.argv) < 2:
        print("Usage: python3 dynamic_obstacle_evaluation.py [model_path]")
        sys.exit(0) 
    ...
    agent = PPOAgent(env_name="Evaluate-Dynamic-Obstacle",
    ...
    # CSVファイルに結果を書き出す
    dir_name = f"./ppo_Evaluate-Dynamic-Obstacle/{agent.start_time}"
    # ディレクトリを作成
    os.makedirs(dir_name, exist_ok=True)
    filename = f'{dir_name}/dynamic_obstacle_results.csv'
